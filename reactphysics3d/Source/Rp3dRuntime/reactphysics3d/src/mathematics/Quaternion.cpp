    ...
    if (trace < decimal(0.0)) {
        if (matrix[1][1] > matrix[0][0]) {
            if(matrix[2][2] > matrix[1][1]) {
                r = rp3dSqrt(matrix[2][2] - matrix[0][0] - matrix[1][1] + decimal(1.0));
                s = decimal(0.5) / r;
                ...
                r = rp3dSqrt(matrix[1][1] - matrix[2][2] - matrix[0][0] + decimal(1.0));
                s = decimal(0.5) / r;
            ...
            r = rp3dSqrt(matrix[0][0] - matrix[1][1] - matrix[2][2] + decimal(1.0));
            s = decimal(0.5) / r;
...
Quaternion Quaternion::slerp(const Quaternion& quaternion1,
    ...
    decimal theta = rp3dAcos(cosineTheta);

    // Compute sin(theta)
    decimal sineTheta = rp3dSin(theta);

    // Compute the two coefficients that are in the spherical linear interpolation formula
    decimal coeff1 = rp3dSin((decimal(1.0)-t)*theta) / sineTheta;
    decimal coeff2 = rp3dSin(t*theta) / sineTheta * invert;
...
void Quaternion::initWithEulerAngles(decimal angleX, decimal angleY, decimal angleZ) {
    ...
    const decimal sinX = rp3dSin(angle);
    const decimal cosX = rp3dCos(angle);

    angle = angleY * decimal(0.5);
    const decimal sinY = rp3dSin(angle);
    const decimal cosY = rp3dCos(angle);

    angle = angleZ * decimal(0.5);
    const decimal sinZ = rp3dSin(angle);
    const decimal cosZ = rp3dCos(angle);
